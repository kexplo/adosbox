 ...
 */
	...
	CASE_B(0x67)												/* Address Size Prefix */
		// fallback from verysimple to full
		core.cseip--; // rollback this prefix
		if (TEST_PREFIX_REP) core.cseip--; // rollback for possible?? REP prefix
		cpudecoder = &CPU_Core_Simple_Run;
		LOG_MSG("Fallback to real Simple core");
		goto decode_end; // switch cores
	CASE_W(0x68)												/* PUSH Iw */
