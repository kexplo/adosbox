...
#define SDATATYPE Bit16u
#define DSTDATATYPE Bit16u
#define CHKPAL CheckPaletteBit32u
#define SCHKPAL CheckPaletteBit16u
#endif
...
#define DrawToolbar \
	...
	if (process & 2) { \
		...
		Bit32u* dst = (Bit32u*)Scaler_DstWrite; \
		Bit32u* tb = (Bit32u*)&toolb + (tbscrollpos + Scaler_Line - MH)*320/2; \
		x=320/2; \
   	do { \
   	  Bit32u px1 = *(src++); \
   	  Bit32u px2 = *(tb++); \
   	  *(dst++)=((px1 & 0xF7DEF7DE)>>1) + ((px2 & 0xF7DEF7DE)>>1); \
	  } while (--x); \
	}
...
#define GetSourceData(DataType, CheckPalette) \
	...
	if (out == *Cache && !(process & 8) && !(process & 4 && (CheckPalette)) ) { \
		Cache++; \
		process &= ~1L; \
	} \
	else { \
...
static void conc3d(NormalGX,SBPP,DBPP) (const void * src) {
	...
	while (x < ModeWidth) {
		...
		if (!(process & 3))
			...
			*dst=PMAKE(out & 0xFF);
			dst+=Pitch;
			*dst=PMAKE((out >> 8) & 0xFF);
			dst+=Pitch;
			*dst=PMAKE((out >> 16) & 0xFF);
			dst+=Pitch;
			*dst=PMAKE(out >> 24);
			dst+=Pitch;
		}
...
static void conc3d(HalfGX,SBPP,DBPP) (const void * src) {
	...
	while (x < ModeWidth) {
		...
		if (!(process & 3))
			...
			*dst=((PMAKE(out & 0xFF) & 0xF7DE) + (PMAKE((out >> 8) & 0xFF) & 0xF7DE)) >> 1;
			dst+=Pitch;
			*dst=((PMAKE((out >> 16) & 0xFF) & 0xF7DE) + (PMAKE(out >> 24) & 0xF7DE)) >> 1;
			dst+=Pitch;
		}
	}
	EndScalerGX
...
static void conc3d(SpecialGX,SBPP,DBPP) (const void * src) {
	BeginScalerGX(SDATATYPE, DSTDATATYPE)
	Bitu dx = (Scaler_SrcWidth - ModeWidth);
	Bitu xx = 0;
	while (x < ModeWidth) {
		xx+=2*dx;
		GetSourceData(SDATATYPE, SCHKPAL)
		if (process & 3) {
			Bit32u px1 = PMAKE(out & 0xFF);
			Bit32u px2 = PMAKE(out >> 8);
			if (xx >= ModeWidth) {
