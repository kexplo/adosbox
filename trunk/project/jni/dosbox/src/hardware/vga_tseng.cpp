...
void FinishSetMode_ET4K(Bitu crtc_base, VGA_ModeExtraData* modeData) {
	...
	if (modeData->modeNo > 0x13) {
		...
		Bits dist = 100000000;
		for (Bitu i=0; i<16; i++) {
		  if (abs((long int)(target-et4k.clockFreq[i])) < dist) {
				best = i;
				dist = abs((long int)(target-et4k.clockFreq[i]));
			}
		}
...
void FinishSetMode_ET3K(Bitu crtc_base, VGA_ModeExtraData* modeData) {
	...
	if (modeData->modeNo > 0x13) {
		...
		Bits dist = 100000000;
		for (Bitu i=0; i<8; i++) {
		  if (abs((long int)(target-et3k.clockFreq[i])) < dist) {
				best = i;
				dist = abs((long int)(target-et3k.clockFreq[i]));
			}
		}
