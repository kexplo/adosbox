...
Bit8u UsedBlocks[SBNUM];  

Bit8u SmallBlocks[SBSIZE*SBNUM];
...
{
allocated--;
        if (addr>&SmallBlocks[0])
           if (addr<&SmallBlocks[SBSIZE*SBNUM]) {
		Bit32u i = ((Bit32u)addr-(Bit32u)&SmallBlocks[0])/SBSIZE;
		UsedBlocks[i]=0;
			LOG_MSG("SB-: %d",i);
		return;
	}
	...
	LOG_MSG("Free: %x,%d", addr,allocated);
	if (H==0)
	{
	free((char*)addr-4);
	return;
...
public:
	...
	void writeb(PhysPt addr,Bitu val){
		...
		if (!host_readb(&write_map[addr])) {
			...
			invalidation_map=(Bit8u*)mymalloc(4096);
...
static void cache_init(bool enable) {
	...
	if (enable) {
		...
		if (cache_blocks == NULL) {
			...
			cache_blocks=(CacheBlockDynRec*)mymalloc(CACHE_BLOCKS*sizeof(CacheBlockDynRec));
		...
		if (cache_code_start_ptr==NULL) {
			...
			if (!cache_code_start_ptr)
				cache_code_start_ptr=(Bit8u*)mymalloc(CACHE_TOTAL+CACHE_MAXSIZE+PAGESIZE_TEMP-1+PAGESIZE_TEMP);
#else
			cache_code_start_ptr=(Bit8u*)mymalloc(CACHE_TOTAL+CACHE_MAXSIZE+PAGESIZE_TEMP-1+PAGESIZE_TEMP);
#endif
