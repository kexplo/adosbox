...
#define TLB_SIZE		(1024*1024)/TLB_CROP
#else
...
#define PAGING_LINKS (128*1024/4)/TLB_CROP
...
INLINE Bit8u mem_readb_inline(PhysPt address) {
	...
	if (tlb_addr) return *(Bit8u*)(tlb_addr+address);
...
INLINE void mem_writeb_inline(PhysPt address,Bit8u val) {
	...
	if (tlb_addr) *(Bit8u*)(tlb_addr+address)=val;
