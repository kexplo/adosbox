...
static Bit16u decode_fetchw(void) {
	...
	if (decode.page.index&1) {
               ...
               *(Bit8u *)&decode.page.wmap[decode.page.index+1]+=0x01;
	} else *(Bit16u *)&decode.page.wmap[decode.page.index]+=0x0101;
...
static Bit32u decode_fetchd(void) {
	...
	if (decode.page.index&3) {
               *(Bit8u *)&decode.page.wmap[decode.page.index]+=0x01;
               *(Bit8u *)&decode.page.wmap[decode.page.index+1]+=0x01;
               *(Bit8u *)&decode.page.wmap[decode.page.index+2]+=0x01;
               *(Bit8u *)&decode.page.wmap[decode.page.index+3]+=0x01;
        } else *(Bit32u *)&decode.page.wmap[decode.page.index]+=0x01010101;
...
static void INLINE decode_increase_wmapmask(Bitu size) {
	...
	if (GCC_UNLIKELY(!activecb->cache.wmapmask)) {
		...
		activecb->cache.wmapmask=(Bit8u*)mymalloc(START_WMMEM);
	...
	switch (size) {
		...
		case 2 :
			if (mapidx&1) {
				...
				(*(Bit8u*)&activecb->cache.wmapmask[mapidx+1])+=0x01; 
		...
		case 4 : 
			if (mapidx&3) {
				(*(Bit8u*)&activecb->cache.wmapmask[mapidx])+=0x01; 
				(*(Bit8u*)&activecb->cache.wmapmask[mapidx+1])+=0x01; 
				(*(Bit8u*)&activecb->cache.wmapmask[mapidx+2])+=0x01; 
				(*(Bit8u*)&activecb->cache.wmapmask[mapidx+3])+=0x01; 
